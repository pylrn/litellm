...
def _new_sync_client(uri: str, token: str, db_name: str, timeout: float | None) -> _SyncMilvusClient:
    ...
    return typing.cast(  # noqa: TID251  # cast-ok: pymilvus lacks typing metadata; protocol defines the used surface
        _SyncMilvusClient,
        MilvusClient(uri=uri, token=token, db_name=db_name, timeout=timeout),
    )
...
def _new_async_client(uri: str, token: str, db_name: str, timeout: float | None) -> _AsyncMilvusClient:
    ...
    return typing.cast(  # noqa: TID251  # cast-ok: pymilvus lacks typing metadata; protocol defines the used surface
        _AsyncMilvusClient,
        AsyncMilvusClient(uri=uri, token=token, db_name=db_name, timeout=timeout),
    )
...
class _MilvusSearchOptions(BaseModel):
    ...
    @classmethod
    def reject_openai_filters(cls, value: Mapping[str, object] | None) -> None:
        if value is not None:
            raise ValueError(
                "Milvus gRPC search does not support the filters parameter yet; use the Milvus filter option"
            )
...
class MilvusGRPCVectorStoreConfig(BaseDirectVectorStoreConfig):
    ...
    @staticmethod
    def _timeout_seconds(timeout: float | httpx.Timeout | None) -> float | None:
    ...
    def execute_search_vector_store_request(
        ...
        options: Final = _MilvusSearchOptions.model_validate(vector_store_search_optional_params)
        ...
        timeout_seconds: Final = self._timeout_seconds(timeout)
        ...
        client: Final = _new_sync_client(params.uri, params.token, params.db_name, timeout_seconds)
        try:
    ...
    async def aexecute_search_vector_store_request(
        ...
        client: Final = _new_async_client(params.uri, params.token, params.db_name, timeout_seconds)
        try:
